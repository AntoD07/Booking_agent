...
def test_generate_french_draft_moves_card_to_draft_ready(auth_client, monkeypatch):
    _no_key(monkeypatch)
    with SessionLocal() as db:
        venue = _make_venue(db, name="Jazz à Vienne", country="France")
        vid = venue.id
...
def test_generate_english_for_non_francophone(auth_client, monkeypatch):
    _no_key(monkeypatch)
    with SessionLocal() as db:
        venue = _make_venue(db, name="Fasching", country="Sweden", type=VenueType.jazz_club)
        vid = venue.id
...
def test_generate_uses_profile_links_and_greeting(auth_client, monkeypatch):
    _no_key(monkeypatch)
    auth_client.put(
        "/api/band-profile",
        json={
    ...
    with SessionLocal() as db:
        venue = _make_venue(db, booking_contact="Marie Dupont, programmation")
        vid = venue.id
    body = auth_client.post(f"/api/venues/{vid}/drafts").json()["body"]
...
def test_patch_body_and_mark_sent_advances_venue(auth_client, monkeypatch):
    _no_key(monkeypatch)
    with SessionLocal() as db:
        venue = _make_venue(db, status=VenueStatus.draft_ready)
        vid = venue.id
...
def test_mark_sent_does_not_drag_back_a_confirmed_venue(auth_client, monkeypatch):
    _no_key(monkeypatch)
    with SessionLocal() as db:
        venue = _make_venue(db, status=VenueStatus.confirmed)
        vid = venue.id
    draft_id = auth_client.post(f"/api/venues/{vid}/drafts").json()["id"]
...
def test_patch_blank_body_rejected(auth_client, monkeypatch):
    _no_key(monkeypatch)
    with SessionLocal() as db:
        vid = _make_venue(db).id
    draft_id = auth_client.post(f"/api/venues/{vid}/drafts").json()["id"]
    assert auth_client.patch(f"/api/drafts/{draft_id}", json={"body": "  "}).status_code == 422


def test_list_and_delete_drafts(auth_client, monkeypatch):
    _no_key(monkeypatch)
    with SessionLocal() as db:
        vid = _make_venue(db).id
...
def test_appearances_feed_the_hook_prompt(client):
    with SessionLocal() as db:
        venue = _make_venue(db)
        artist_link = VenueArtist(venue_id=venue.id, year="2019")
        from app.models import Artist

        artist = Artist(name="Rosenberg Trio")
        db.add(artist)
        db.flush()
...
def test_edition_year_prefers_future_event_year(client):
    with SessionLocal() as db:
        venue = _make_venue(db, event_dates="24-27 June 2028")
        assert drafting._edition_year(venue) == 2028
        past = _make_venue(db, name="Old", event_dates="3-18 July 2026")
